#!/bin/bash
# reap_prune_v6.sh - Production with fast pip installs
...
if [ ! -d ".venv" ]; then
    ...
    log "Installing torch (pip, fast)..."
    pip install --no-cache-dir torch --index-url https://download.pytorch.org/whl/cu124 >> "$LOG_FILE" 2>&1
    
    log "Installing dependencies..."
    pip install --no-cache-dir transformers accelerate datasets pyyaml >> "$LOG_FILE" 2>&1
