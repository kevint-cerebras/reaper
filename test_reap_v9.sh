#!/bin/bash
# test_reap_v9.sh - Supports both Qwen2Moe (test) and Qwen3Next (production)
...
echo "→ Configuring Qwen models..."
python << 'EOF'
# Add Qwen2Moe (test model) and Qwen3Next (production model)
with open('src/reap/model_util.py', 'r') as f:
    content = f.read()

if 'Qwen2MoeForCausalLM' not in content:
    qwen2_config = '''    "Qwen2MoeForCausalLM": {
        "moe_block": "mlp",
        "gate_proj": "gate_proj",
        "up_proj": "up_proj",
        "down_proj": "down_proj",
...
'''
    content = content.replace('"Qwen3MoeForCausalLM": {', qwen2_config + '"Qwen3MoeForCausalLM": {')

if 'Qwen3NextForCausalLM' not in content:
    qwen3next_config = '''    "Qwen3NextForCausalLM": {
        "moe_block": "mlp",
        "gate_proj": "gate_proj",
        "up_proj": "up_proj",
        "down_proj": "down_proj",
...
'''
    content = content.replace('"Qwen3MoeForCausalLM": {', qwen3next_config + '"Qwen3MoeForCausalLM": {')
